...
ny=128
nz=128
region_ijk=(60 68 60 68 1 128)
tstart=100
tend=500
tstride=100
tcount=$(((${tend}-${tstart})/${tstride}+1))
data_dir='.'
file_mesh='mesh_128-128-128.h5'
file_field='monitor_region_'
file_xdmf='visu_region.xdmf'
...
for ((i=0; i<3; i++))
do
    region_size[$i]=$(( ${region_ijk[$((2*i+1))]} - ${region_ijk[$((2*i))]} + 1 ))
done
...
tag_start 'Xdmf Version="2.0"'
    tag_start 'Domain'
        tag 'Topology name="Topo" TopologyType="3DRectMesh" Dimensions="'${region_size[2]}' '${region_size[1]}' '${region_size[0]}'"'
        tag_start 'Geometry name="Geo" Type="VXVYVZ"'
            tag_start 'DataItem ItemType="HyperSlab" Dimensions="'${region_size[0]}'"'
                tag_start 'DataItem Dimensions="3 1" Format="XML"'
                    data $((region_ijk[0]-1))
                    data 1
                    data ${region_size[0]}
                tag_end   'DataItem'
                ...
                tag_end   'DataItem'
            tag_end   'DataItem'
            tag_start 'DataItem ItemType="HyperSlab" Dimensions="'${region_size[1]}'"'
                tag_start 'DataItem Dimensions="3 1" Format="XML"'
                    data $((region_ijk[2]-1))
                    data 1
                    data ${region_size[1]}
                ...
                tag_end   'DataItem'
            tag_end   'DataItem'
            tag_start 'DataItem ItemType="HyperSlab" Dimensions="'${region_size[2]}'"'
                tag_start 'DataItem Dimensions="3 1" Format="XML"'
                    data $((region_ijk[4]-1))
                    data 1
                    data ${region_size[2]}
                ...
                tag_start 'Grid Name="'${t_fmt}'" GridType="Uniform"'
                    ...
                    do
                        tag_start 'Attribute Name="'${name}'" Center="Node"'
                            tag_start 'DataItem Dimensions="'${region_size[2]}' '${region_size[1]}' '${region_size[0]}'" NumberType="Float" Precision="'${fp}'" Format="HDF"'
                                data ${file_field}${t_fmt}'.h5:/'${name}''
                            tag_end   'DataItem'
                        tag_end   'Attribute'
                    done
                tag_end   'Grid'
